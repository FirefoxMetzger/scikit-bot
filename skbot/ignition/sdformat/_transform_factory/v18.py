...
def resolve_include(include: IncludeElement, graph: Graph) -> Graph:
...
def convert_world(world: v18.World) -> Graph:
    graph = Graph()
...
def convert_model(model: v18.ModelModel, *, graph: Graph = None) -> Graph:
    if graph is None:
        graph = Graph()
...
def converter(sdf: str, *, unwrap=True) -> Graph:
    ...
    for world in sdf_root.world:
        graph = convert_world(world)
        graph.root_node = world.name
        graph_list.append(graph)

    if sdf_root.model is not None:
        graph = convert_model(sdf_root.model)
