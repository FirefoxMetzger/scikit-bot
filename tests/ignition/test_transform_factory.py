...
def test_v18_parsing(v18_sdf):
    try:
        frame = ign.to_frame_graph(v18_sdf)
...
def test_v18_parsing_wrapped(v18_sdf):
    try:
        frame_list = ign.to_frame_graph(v18_sdf, unwrap=False)
    except NotImplementedError:
        pytest.skip("Elements not implemented yet.")
...
def test_v18_refuted(v18_sdf_refuted):
    with pytest.raises(ign.sdformat.sdformat.ParseError):
        ign.to_frame_graph(v18_sdf_refuted)


def test_v17_parsing(v17_sdf):
    try:
        frame = ign.to_frame_graph(v17_sdf)
    except NotImplementedError:
        pytest.skip("Elements not implemented yet.")
...
def test_v17_refuted(v17_sdf_refuted):
    with pytest.raises(ign.sdformat.sdformat.ParseError):
        ign.to_frame_graph(v17_sdf_refuted)
...
def test_nd_panda():
    ...
    root_frame = ign.to_frame_graph(sdf_string, shape=(5, 3))
