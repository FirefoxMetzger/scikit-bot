...
def test_custom_distance(panda):
    ...
    angles = ik.ccd((0,0,0), root_pos, tool_frame, base_frame, joints, metric=lambda x, y: np.linalg.norm(x-y, ord=4))
    final_pos = tool_frame.transform((0,0,0), base_frame)
...
def test_ccd_linesearch_maxiter(double_pendulum):
    ...
    with pytest.raises(RuntimeError):
        ik.ccd((0,0,0), root_pos, tool_frame, base_frame, joints, line_search_maxiter=1)
